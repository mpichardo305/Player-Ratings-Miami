 ...
 * @param hoursAfterStart Hours after the start time to consider the game ended (default: 2)
 * @returns boolean indicating if the game has ended
 */
export const hasGameEnded = (gameDate: string, gameStartTime: string, hoursAfterStart: number = 2): boolean => {
  const now = new Date();
